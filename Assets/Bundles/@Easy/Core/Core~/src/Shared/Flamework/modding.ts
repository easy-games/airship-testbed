...
export namespace Modding {
	...
	export function registerDependency<T>(dependency: DependencyRegistration, id?: string) {
		...
		if (typeIs(dependency, "function")) {
			dependencyResolution.set(id, dependency as (ctor: Constructor) => object);
		} else {
	...
	export function resolveDependency(
		...
		throw `Could not find constructor for ${dependencyId} while constructing ${ctor}. This is usually caused by yielding inside a controller/service constructor. Make sure you do not yield or error inside a controller/service constructor.`;
	}
