...
interface Collider2D extends Behaviour {
	...
	Cast(
		...
		results: CSArray<RaycastHit2D>,
		distance: number,
		ignoreSiblingColliders: boolean,
	): number;
	Cast(direction: Vector2, results: CSArray<RaycastHit2D>, distance: number, ignoreSiblingColliders: boolean): number;
	Cast(
		direction: Vector2,
...
interface Mesh extends Object {
	...
	AddBlendShapeFrame(
		shapeName: string,
		frameWeight: number,
		deltaVertices: CSArray<Vector3>,
		deltaNormals: CSArray<Vector3>,
		deltaTangents: CSArray<Vector3>,
	): void;
	Clear(keepVertexLayout: boolean): void;
	Clear(): void;
	ClearBlendShapes(): void;
	CombineMeshes(
		combine: CSArray<CombineInstance>,
	...
	SetIndexBufferData<T>(
		data: CSArray<T>,
		dataStart: number,
		meshBufferStart: number,
		count: number,
		flags: MeshUpdateFlags,
	): void;
	SetIndexBufferData<T>(
		data: CSArray<T>,
		dataStart: number,
...
interface GraphicsBuffer {
	...
	SetData<T>(
		data: CSArray<T>,
		managedBufferStartIndex: number,
		graphicsBufferStartIndex: number,
		count: number,
	): void;
...
interface Physics2DConstructor {
	...
	GetContacts(
		...
		contactFilter: ContactFilter2D,
		contacts: CSArray<ContactPoint2D>,
	): number;
...
interface Texture2D extends Texture {
	...
	PackTextures(
		textures: CSArray<Texture2D>,
		padding: number,
		maximumAtlasSize: number,
		makeNoLongerReadable: boolean,
	): CSArray<Rect>;
...
interface SpriteConstructor {
	...
	Create(
		...
		border: Vector4,
		generateFallbackPhysicsShape: boolean,
		secondaryTextures: CSArray<SecondarySpriteTexture>,
	): Sprite;
	Create(
		texture: Texture2D,
		rect: Rect,
...
interface ServerManager extends MonoBehaviour {
	...
	BroadcastExcept<T>(
		connections: CSArray<NetworkConnection>,
		excludedConnections: CSArray<NetworkConnection>,
		message: T,
		requireAuthenticated: boolean,
		channel: Channel,
	): void;
...
interface NetworkBehaviour extends MonoBehaviour {
	...
	Reconcile_Client<T, T2>(
		reconcileDel: unknown,
		replicateULDel: unknown,
		replicates: CSArray<T2>,
		data: T,
		channel: Channel,
	): void;
	...
	Replicate_NonOwner<T>(
		del: unknown,
		q: unknown,
		serverControlData: T,
		allowServerControl: boolean,
		channel: Channel,
	): void;
...
interface Reader {
	...
	ReadNetworkBehaviour(
		...
		readSpawningObjects: CSArray<number>,
	): NetworkBehaviour;
...
interface RenderTextureConstructor {
	...
	new (
		...
		depth: number,
		format: RenderTextureFormat,
		readWrite: RenderTextureReadWrite,
	): RenderTexture;
...
interface CommandBuffer {
	...
	BeginRenderPass(
		...
		height: number,
		samples: number,
		attachments: CSArray<AttachmentDescriptor>,
		depthAttachmentIndex: number,
		subPasses: CSArray<SubPassDescriptor>,
	): void;
	BeginRenderPass(
		width: number,
		height: number,
	...
	Blit(
		...
		sourceDepthSlice: number,
		destDepthSlice: number,
	): void;
	Blit(
		source: RenderTargetIdentifier,
		dest: RenderTargetIdentifier,
	...
	ConvertTexture(
		src: RenderTargetIdentifier,
		srcElement: number,
		dst: RenderTargetIdentifier,
		dstElement: number,
	): void;
	...
	DispatchCompute(
		...
		threadGroupsZ: number,
	): void;
	DispatchCompute(
		computeShader: ComputeShader,
		kernelIndex: number,
		indirectBuffer: ComputeBuffer,
		argsOffset: number,
	): void;
	DispatchCompute(
		computeShader: ComputeShader,
		kernelIndex: number,
		indirectBuffer: GraphicsBuffer,
		argsOffset: number,
	): void;
	DispatchRays(
	...
	IssuePluginEventAndDataWithFlags(
		callback: unknown,
		eventID: number,
		flags: CustomMarkerCallbackFlags,
		data: unknown,
	): void;
	...
	RequestAsyncReadbackIntoNativeSlice<T>(
		output: CSArray<T>,
		src: ComputeBuffer,
		size: number,
		offset: number,
	...
	SetComputeTextureParam(
		...
		rt: RenderTargetIdentifier,
		mipLevel: number,
		element: RenderTextureSubElement,
	): void;
	SetComputeTextureParam(
		computeShader: ComputeShader,
		kernelIndex: number,
	...
	SetRenderTarget(
		rt: RenderTargetIdentifier,
		colorLoadAction: RenderBufferLoadAction,
	...
	SetRenderTarget(
		colors: CSArray<RenderTargetIdentifier>,
		depth: RenderTargetIdentifier,
		mipLevel: number,
		cubemapFace: CubemapFace,
		depthSlice: number,
...
interface RayTracingAccelerationStructure {
	AddInstance(
		...
		subMeshFlags: CSArray<number>,
		enableTriangleCulling: boolean,
		frontTriangleCounterClockwise: boolean,
		mask: number,
		id: number,
	): number;
	...
	AddInstance(
		targetRenderer: Renderer,
		subMeshMask: CSArray<boolean>,
		subMeshTransparencyFlags: CSArray<boolean>,
		enableTriangleCulling: boolean,
		frontTriangleCounterClockwise: boolean,
		mask: number,
		id: number,
...
interface MaterialPropertyBlock {
	...
	CopyProbeOcclusionArrayFrom(
		occlusionProbes: CSArray<Vector4>,
		sourceStart: number,
		destStart: number,
		count: number,
	): void;
	...
	CopySHCoefficientArraysFrom(
		lightProbes: CSArray<SphericalHarmonicsL2>,
		sourceStart: number,
		destStart: number,
		count: number,
	): void;
...
interface CameraConstructor {
	...
	CalculateProjectionMatrixFromPhysicalProperties(
		...
		nearClip: number,
		farClip: number,
		gateFitParameters: GateFitParameters,
	): void;
...
interface CanvasRendererConstructor {
	...
	AddUIVertexStream(
		verts: CSArray<UIVertex>,
		positions: CSArray<Vector3>,
		colors: CSArray<Color32>,
		uv0S: CSArray<Vector4>,
		uv1S: CSArray<Vector4>,
		normals: CSArray<Vector3>,
		tangents: CSArray<Vector4>,
	): void;
	AddUIVertexStream(
		...
		uv0S: CSArray<Vector4>,
		uv1S: CSArray<Vector4>,
		uv2S: CSArray<Vector4>,
		uv3S: CSArray<Vector4>,
		normals: CSArray<Vector3>,
		tangents: CSArray<Vector4>,
	): void;
...
interface Animator extends Behaviour {
	...
	CrossFadeInFixedTime(
		stateName: string,
		fixedTransitionDuration: number,
		layer: number,
		fixedTimeOffset: number,
	): void;
	CrossFadeInFixedTime(
		stateName: string,
		fixedTransitionDuration: number,
...
interface StateMachineBehaviour extends ScriptableObject {
	...
	OnStateEnter(
		...
		layerIndex: number,
		controller: AnimatorControllerPlayable,
	): void;
...
interface TMP_FontAsset extends TMP_Asset {
	...
	HasCharacters(
		text: string,
		missingCharacters: CSArray<number>,
		searchFallbacks: boolean,
		tryAddCharacter: boolean,
	): boolean;
	...
	TryAddCharacters(
		unicodes: CSArray<number>,
		missingUnicodes: CSArray<number>,
...
interface TMP_FontAssetConstructor {
	...
	CreateFontAsset(
		...
		atlasHeight: number,
		atlasPopulationMode: AtlasPopulationMode,
		enableMultiAtlasSupport: boolean,
	): TMP_FontAsset;
...
interface MaterialReferenceConstructor {
	...
	AddMaterialReference(
		material: Material,
		fontAsset: TMP_FontAsset,
		materialReferences: CSArray<MaterialReference>,
		materialReferenceIndexLookup: CSDictionary<number, number>,
	): number;
	AddMaterialReference(
		material: Material,
		spriteAsset: TMP_SpriteAsset,
...
interface Light extends Behaviour {
	...
	AddCommandBufferAsync(
		...
		shadowPassMask: ShadowMapPass,
		queueType: ComputeQueueType,
	): void;
...
interface ParticleSystemRenderer extends Renderer {
	...
	BakeTexture(
		verticesTexture: unknown,
		indicesTexture: unknown,
		camera: Camera,
		options: ParticleSystemBakeTextureOptions,
	): number;
...
interface TexturePacker {
	...
	PackTextures(
		textures: CSDictionary<string, TextureSet>,
