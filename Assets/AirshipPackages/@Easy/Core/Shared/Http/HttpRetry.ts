...
type RateLimitInstruction =
	...
	| RateLimitRetryInstruction
	| RateLimitSuccessInstruction;
	...
	try {
		...
		switch (instruction.type) {
			case RateLimitInstructionType.PassThroughError:
				...
				if (retryKey !== undefined) {
					CoreLogger.Warn(
						`Http request "${retryKey}" received a 429 response code but could not determine a retry time.`,
					);
				} else {
					CoreLogger.Warn(
						"Http request without a retry key received a 429 response code but could not determine a retry time.",
					);
				}
			...
			case RateLimitInstructionType.Retry:
				...
				if (retryKey !== undefined) {
					CoreLogger.Warn(
						`Http request "${retryKey}" was rate limited for ${instruction.resetAfter} seconds.`,
					);
				} else {
					CoreLogger.Warn(
						`Http request without a retry key was rate limited for ${instruction.resetAfter} seconds.`,
					);
				}
...
export type HttpCallback = () => HttpResponse;
export type HttpRetryCallback = ((httpRequest: HttpCallback, config?: HttpRetryConfig) => Promise<HttpResponse>) &
	((httpRequest: HttpCallback, retryKey: string) => Promise<HttpResponse>);

const CreateHttpRetryPromise: HttpRetryCallback = (
	httpRequest: HttpCallback,
	config: HttpRetryConfig | string = {},
