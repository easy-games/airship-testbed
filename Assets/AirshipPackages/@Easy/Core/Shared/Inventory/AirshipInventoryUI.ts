...
export default class AirshipInventoryUI extends AirshipBehaviour {
	...
	private BindDragEventsOnButton(button: Button, inventory: Inventory, slotIndex: number): EngineEventConnection[] {
		return [
			...
			CanvasAPI.OnPointerEvent(button.gameObject, (direction, pointerButton) => {
					...
					if (this.clickPickupState && this.clickPickupState.isFirstClickDrag) {
						...
						if (endButton) {
							...
							if (endSlotIndex !== undefined && endInventory) {
								const pickupFromInv =
									this.clickPickupState.sourceInventory ?? this.clickPickupState.inventory;
								const isDifferentButton =
									endSlotIndex !== this.clickPickupState.slot ||
									endInventory !== pickupFromInv;
				...
				const isSameSlot =
					targetSlotIndex === this.clickPickupState.slot && inventory === pickupFromInv;
				if (
	...
	private HandleItemPlacementOnDragEnd(
		...
		const isSameSlotAsPickup =
			targetSlotIndex === this.clickPickupState.slot && targetInventory === pickupFromInv;
		if (existingItemStack && (!isSameSlotAsPickup || this.clickPickupState.swapStack)) {
			if (existingItemStack.itemType === this.clickPickupState.itemType) {
	...
	private HandleItemPickup(
		...
		const playerInv =
			Airship.Inventory.localInventory ?? Game.localPlayer?.character?.inventory;
		const isFromExternal = inventory === this.externalInventory;
		const pickupInv = isFromExternal && playerInv ? playerInv : inventory;
