...
export class AvatarUtil {
  ...
  //@Easy/Core/Shared/Resources/Accessories/AvatarItems/GothGirl/Kit_GothGirl_Collection.asset
  private static readonly allAvatarAccessories = new Map<
    string,
    AccessoryComponent
  >();
  ...
  public static Initialize() {
    ...
    let avatarCollection =
      AssetBridge.Instance.LoadAsset<AvatarAccessoryCollection>(
    ...
    for (let i = 0; i < avatarCollection.accessories.Length; i++) {
      ...
      if (!element) {
        CoreLogger.Warn(
          "Empty element in avatar generalAccessories collection: " + i
        );
        continue;
      }
  ...
  public static InitUserOutfits(userId: string) {
    ...
    if (
      !outfits ||
      outfits.size() === 0 ||
      AvatarPlatformAPI.GetEquippedOutfit() === undefined
    ) {
      AvatarPlatformAPI.EquipAvatarOutfit(name);
    }
  }

  public static AddAvailableAvatarItem(
    instanceId: string,
  ...
  public static LoadUserOutfit(
    ...
    outfit.accessories.forEach((acc) => {
      const accComponentTemplate = this.GetAccessoryFromClassId(
        acc.class.classId
      );
      if (accComponentTemplate) {
        let accComponent = builder.AddSingleAccessory(
          accComponentTemplate,
          false
        );
