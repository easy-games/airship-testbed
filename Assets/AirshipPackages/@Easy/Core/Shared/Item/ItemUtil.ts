...
export class ItemUtil {
  public static readonly defaultItemPath =
    "@Easy/Core/Shared/Resources/Accessories/missing_item.prefab";

  private static readonly itemAccessories = new Map<
    string,
    AccessoryComponent[]
  >();
  ...
  public static Initialize() {
    //Load default items
    ItemUtil.missingItemAccessory =
      AssetBridge.Instance.LoadAsset<AccessoryComponent>(
        ItemUtil.defaultItemPath
      );
  ...
  public static RegisterItem(
    ...
    if (config?.accessoryFolder) {
      if (itemDefinition.accessoryPaths) {
        ...
        itemDefinition.accessoryPaths = [
          config.accessoryFolder + "/" + itemType.lower() + ".prefab",
        ];
      }
    }
    ...
    if (accessoryPaths.size() > 0) {
      ...
      for (const accessoryName of accessoryPaths) {
        let accessory =
          AssetBridge.Instance.LoadAssetIfExists<GameObject>(accessoryName);
        if (!accessory) {
          continue;
        }
  ...
  public static GetItemTypeFromBlockId(blockId: number): string | undefined {
    const WorldAPI = import("@Easy/Core/Shared/VoxelWorld/WorldAPI").expect()
      .WorldAPI;
  ...
  public static GetFirstAccessoryForItemType(
    itemType: string
  ): AccessoryComponent {
  ...
  public static GetAccessoriesForItemType(
    itemType: string
  ): Readonly<AccessoryComponent[]> {
    let accessories = this.itemAccessories.get(itemType);
   ...
   */
  public static FindItemTypeFromExpression(
    expression: string
  ): string | undefined {
    if (CoreItemDefinitions[expression] !== undefined)
   ...
   */
  public static GetItemTypeComponents(
    itemType: string
