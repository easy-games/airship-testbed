...
export class CameraSystem {
  ...
  private updateTransformCallbacks = new Set<
    (transform: CameraTransform) => CameraTransform | void
  >();
  ...
  constructor() {
    ...
    for (const cameraType of ObjectUtils.values(CharacterCameraType)) {
      this.fovStateMap.set(cameraType, {
        fovSpring: new Spring(
          new Vector3(this.mainCamera.fieldOfView, 0, 0),
          5
        ),
  ...
  private OnEnabled() {
    const stopOnUpdate = OnUpdate.ConnectWithPriority(
    ...
    const stopOnLateUpdate = OnLateUpdate.ConnectWithPriority(
  ...
  public ClearMode() {
    if (this.onClearCallback) {
      ...
      this.SetMode(
        new StaticCameraMode(this.transform.position, this.transform.rotation)
      );
  ...
  private UpdateFOVSpring(
    ...
    if (
      Time.time - fovState.fovSpringMovingStart > 2 &&
      math.abs(fovState.fovSpring.velocity.x) < 0.01
    ) {
      fovState.fovSpringMoving = false;
    }
  }
   ...
   */
  public OnUpdateTransform(
    callback: (cameraTransform: CameraTransform) => CameraTransform | void
  ): () => void {
