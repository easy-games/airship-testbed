...
export class AirshipCharacterCameraSingleton implements OnStart {
  ...
  OnStart(): void {
    Dependency<LocalCharacterSingleton>().stateChanged.Connect((state) => {
      const isSprinting =
        Dependency<LocalCharacterSingleton>().input?.IsSprinting();
   ...
   * @internal
   */
  public UpdateLocalCharacterState(
    stateUpdate: Partial<CharacterStateSnapshot>
  ) {
    let didUpdate = false;
    ...
    this.characterState = ObjectUtils.assign<
      CharacterStateSnapshot,
      Partial<CharacterStateSnapshot>
    >(this.characterState, stateUpdate);
  ...
  private MakeFOVReflectCharacterState(): void {
    ...
    {
      let fov =
        this.overrideFOV.get(CharacterCameraType.FIRST_PERSON) ??
        this.firstPersonFOV;
    ...
    {
      let fov =
        this.overrideFOV.get(CharacterCameraType.THIRD_PERSON) ??
        this.thirdPersonFOV;
      if (this.characterState?.sprinting) {
        fov *= this.sprintFovMultiplier;
      }
  ...
  public ObserveFirstPerson(
    observer: (isFirstPerson: boolean) => CleanupFunc
  ): () => void {
    ...
    keyboard.OnKeyDown(Key.P, (event) => {
      ...
      if (keyboard.IsKeyDown(Key.LeftShift)) {
        if (flyCam) {
          ...
          flyingBin.Add(
            Dependency<LocalCharacterSingleton>().input!.AddDisabler()
          );
          flyingBin.Add(
            Airship.inventory.localCharacterInventory.AddDisabler()
          );
        }
      }
    });
  ...
  private SetLookBackwards(lookBackwards: boolean) {
    ...
    if (
      Dependency<AirshipCharacterCameraSingleton>().cameraSystem?.GetMode() ===
      this.humanoidCameraMode
    ) {
      this.humanoidCameraMode?.SetLookBackwards(this.lookBackwards);
    }
  }
  ...
  public ObserveLookBackwards(
    observer: (lookBackwards: boolean) => CleanupFunc
  ): () => void {
    let currentCleanup: CleanupFunc;
