...
import {
  ...
  OnStart,
  Service,
} from "@Easy/Core/Shared/Flamework";
...
export class PlayersSingleton implements OnStart {
  ...
  constructor() {
    ...
    const FetchLocalPlayerWithWait = () => {
      let localPlayerInfo: PlayerInfo | undefined =
        this.playerManagerBridge.localPlayer;
      ...
      mutable.networkObject =
        localPlayerInfo.gameObject.GetComponent<NetworkObject>()!;
    ...
    if (Game.IsGameLuauContext()) {
        ...
        if (Game.IsServer() && this.joinMessagesEnabled) {
          Game.BroadcastMessage(
            ChatColor.Aqua(player.username) +
              ChatColor.Gray(" joined the server.")
          );
        }
      });
      this.onPlayerDisconnected.Connect((player) => {
        if (Game.IsServer() && this.disconnectMessagesEnabled) {
          Game.BroadcastMessage(
            ChatColor.Aqua(player.username) + ChatColor.Gray(" disconnected.")
          );
        }
      });
    }
  ...
  private InitClient(): void {
    CoreNetwork.ServerToClient.ServerInfo.client.OnServerEvent(
      (gameId, serverId, organizationId) => {
        ...
        if (authenticated) {
          ...
          const disc = contextbridge.subscribe(
            "AuthController:OnAuthenticated",
            (fromContext, args) => {
  ...
  private InitServer(): void {
    ...
    const onPlayerRemoved = (clientInfo: PlayerInfoDto) => {
      ...
      if (player) {
        this.players.delete(player);
        this.onPlayerDisconnected.Fire(player);
        CoreNetwork.ServerToClient.RemovePlayer.server.FireAllClients(
          player.clientId
        );
        player.Destroy();
      }
  ...
  private FetchEquippedOutfit(player: Player, ignoreCache: boolean): void {
    const SetOutfit = (outfitDto: OutfitDto | undefined) => {
      ...
      if (RunUtil.IsEditor()) {
        EditorSessionState.SetString(
          "player_" + player.userId + "_outfit",
          EncodeJSON(outfitDto)
        );
      }
    };

    if (RunUtil.IsEditor() && !ignoreCache) {
      const data = EditorSessionState.GetString(
        "player_" + player.userId + "_outfit"
      );
      if (data) {
    ...
    const res = InternalHttpManager.GetAsync(
      AirshipUrl.ContentService + "/outfits/uid/" + userId + "/equipped"
    );
    if (!res.success) {
  ...
  private HandlePlayerReadyServer(player: Player): void {
    ...
    CoreNetwork.ServerToClient.AddPlayer.server.FireExcept(
      player,
      player.Encode()
    );
  ...
  public FindByClientIdIncludePending(clientId: number): Player | undefined {
    return (
      this.FindByClientId(clientId) ?? this.playersPendingReady.get(clientId)
    );
  }
   ...
   * @internal
   */
  public WaitForClientIdIncludePending(
    clientId: number,
    timeout = 5
