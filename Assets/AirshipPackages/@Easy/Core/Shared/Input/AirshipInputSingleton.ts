...
import {
  ...
  Keyboard,
  Mouse,
  Preferred as PreferredControls,
} from "../UserInput";
...
export class AirshipInputSingleton implements OnStart {
   ...
   *
   */
  private actionDownSignals = new Map<
    string,
    Signal<[event: InputActionEvent]>[]
  >();
  /**
   *
   */
  private actionUpSignals = new Map<
    string,
    Signal<[event: InputActionEvent]>[]
  >();
  /**
    ...
    if (parent) {
      go = Object.Instantiate(
        AssetCache.LoadAsset(
          "@Easy/Core/Shared/Resources/Prefabs/Input/ProximityPrompt.prefab"
        ),
        parent
      );
    } else {
  ...
  public CreateMobileButton(
    ...
    const mobileButtonsForAction =
      this.actionToMobileButtonTable.get(lowerName) ?? [];
  ...
  public ShowMobileButtons(name: string): void {
    const mobileButtonsForAction =
      this.actionToMobileButtonTable.get(name.lower()) ?? [];
    for (const mobileButton of mobileButtonsForAction) {
  ...
  public GetActionByInputType(
    ...
    return actions.find(
      (action) =>
        InputUtil.GetInputTypeFromBinding(action.binding, KeyType.Primary) ===
        inputType
    );
  }
      ...
      if (action.binding.config.isKeyBinding) {
        signalCleanup.Add(
          this.keyboard.OnKeyDown(action.binding.config.key, (event) => {
            const isModifierKeyDown = this.keyboard.IsKeyDown(
              action.binding.GetModifierKey()
            );
            ...
            for (const signal of actionDownSignals) {
              if (signal.HasConnections()) {
                signal.Fire(
                  new InputActionEvent(action.name, event.uiProcessed)
                );
              } else {
                inactiveSignalIndices.push(signalIndex);
              }
        ...
        signalCleanup.Add(
          this.mouse.OnButtonDown(
            action.binding.config.mouseButton,
            (event) => {
              const isModifierKeyDown = this.keyboard.IsKeyDown(
                action.binding.GetModifierKey()
              );
  ...
  public GetMouseSensitivity() {
    return (
      ...
      contextbridge.invoke<() => number>(
        "ClientSettings:GetMouseSensitivity",
