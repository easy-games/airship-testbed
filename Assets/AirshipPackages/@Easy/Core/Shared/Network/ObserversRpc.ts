	...
	return (ctor, property, descriptor) => {
		...
		if (Game.IsServer()) {
			descriptor.value = (object, ...params) => {
				...
				if (isHost) {
					event!.server.FireExcept(Game.localPlayer, object.networkObject.netId, ...(params as never));
				} else {
					event!.server.FireAllClients(object.networkObject.netId, ...(params as never));
				}
