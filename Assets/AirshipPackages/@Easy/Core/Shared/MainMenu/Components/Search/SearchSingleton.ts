import {
  ...
  GamesDto,
  MyGamesDto,
} from "@Easy/Core/Client/Components/HomePage/API/GamesAPI";
import {
  Controller,
  OnStart,
  Service,
} from "@Easy/Core/Shared/Flamework/flamework";
...
export default class SearchSingleton implements OnStart {
  ...
  public FetchMyGames(): void {
    const res = InternalHttpManager.GetAsync(
      AirshipUrl.ContentService + "/memberships/games/self?liveStats=true"
    );
    if (!res.success) {
  ...
  public FetchPopularGames(): void {
    const res = InternalHttpManager.GetAsync(
      AirshipUrl.ContentService + "/games"
    );
    if (!res.success) {
