...
export default class SettingsKeybind extends AirshipBehaviour {
  ...
  private HighlightValueImage(): void {
    this.valueImageBG
      .TweenGraphicColor(new Color(1, 1, 1, 0.5), 0.25)
      .SetPingPong();
  }
  ...
  private StartKeyListener(): void {
    SetInterval(InputPollRate, () => {
      ...
      if (
        ...
        !Keyboard.global.IsKeyDown(this.downModifierKey)
      ) {
      ...
      for (let key of ObjectUtils.keys(InputUtils.keyCodeMap) as Key[]) {
        ...
        if (Keyboard.global.IsKeyDown(key) && this.isListening) {
          ...
          if (modifierKey) {
            ...
            if (
              key !== this.downModifierKey &&
              this.downModifierKey !== Key.None
            ) {
              const modifierKey = InputUtil.GetModifierFromKey(
                this.downModifierKey
              );
  ...
  private UpdateBindingTextFromBinding(binding: Binding): void {
    if (binding.config.isKeyBinding) {
      if (!binding.IsComplexBinding()) {
        const bindingText =
          InputUtils.GetStringForKeyCode(binding.config.key) ??
          `Unknown(${binding.config.key})`;
        this.UpdateKeybindText(bindingText);
      } else {
        const primaryKeyCodeText =
          InputUtils.GetStringForKeyCode(binding.config.key) ??
          `Unknown(${binding.config.key})`;
        const modifierAsKeyCode = InputUtil.GetKeyFromModifier(
          binding.config.modifierKey
        );
        const modifierKeyCodeText =
          InputUtils.GetStringForKeyCode(modifierAsKeyCode) ??
          `Unknown(${modifierAsKeyCode})`;
