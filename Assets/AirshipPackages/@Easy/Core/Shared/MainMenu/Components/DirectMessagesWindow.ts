  ...
  public InitAsFriendChat(user: FriendStatus): void {
    ...
    this.bin.AddEngineEventConnection(
      CanvasAPI.OnClickEvent(this.friendTeleportButton, () => {
        const friend = Dependency<FriendsController>().GetFriendStatus(
          user.userId
        );
        if (friend?.serverId && friend.gameId) {
          print(
            "Transferring to " + user.username + " on server " + friend.serverId
          );
    ...
    const UpdateTeleportButton = (friend: FriendStatus) => {
      let inServer =
        friend.serverId !== undefined && friend.gameId !== undefined;
