...
export namespace ContentServiceDatabaseTypes {
	...
	export const TransactionType = {
		...
		USER_BROKERED_TRADE: "USER_BROKERED_TRADE",
...
export namespace ContentServiceItems {
	...
	export type GetUserInventoryByUserIdArgs = {
		params: {
			uid: string;
		};
		query?: ContentServiceItems.GetItemsDto;
	...
	export interface SelectedItem {
		...
		float: number;
		createdAt: string;
		pendingTradeId: string | undefined;
	}
	...
	export interface ClientSpec {
		...
		getUserInventoryByUserId(
			args: GetUserInventoryByUserIdArgs,
			options?: RequestOptions,
		): Promise<ContentServiceItems.SelectedItem[]>;
	...
	export class Client implements ClientSpec {
		...
		async getUserInventoryByUserId(
			...
			return await this.makeRequest({
				method: "GET",
				routeId: "ContentService:Items:getUserInventoryByUserId",
				path: `/items/uid/${encodeURIComponent(args.params.uid)}`,
				retryKey: options?.retryKey ?? "ContentService:Items:getUserInventoryByUserId",
				query: args.query,
			});
		}
...
export namespace ContentServiceItemTransactions {
	export type DeleteTradeArgs = {
		params: {
			tradeId: string;
		};
	};

	export interface DisplayableTradeRequest {
		id: string;
		contextUid: string;
		createdAt: string;
		itemsGained: ContentServiceItems.SelectedItem[];
		itemsLost: ContentServiceItems.SelectedItem[];
	}

	export type RequestTradeArgs = {
		data: ContentServiceItemTransactions.UserTradeRequestDto;
	};

	export type RespondToTradeArgs = {
		params: {
			tradeId: string;
		};
		data: ContentServiceItemTransactions.UserTradeResponseDto;
	};
	...
	export interface UserTradeRequestDto {
		selfInstanceIds: string[];
		recipientUid: string;
		recipientInstanceIds: string[];
	}
	...
	export interface ClientSpec {
		deleteTrade(args: DeleteTradeArgs["params"], options?: RequestOptions): Promise<void>;
		getTradeRequests(options?: RequestOptions): Promise<{
			incoming: ContentServiceItemTransactions.DisplayableTradeRequest[];
			outgoing: ContentServiceItemTransactions.DisplayableTradeRequest[];
		}>;
		requestTrade(
			args: RequestTradeArgs["data"],
			options?: RequestOptions,
		): Promise<{ request: ContentServiceItemTransactions.DisplayableTradeRequest }>;
		respondToTrade(args: RespondToTradeArgs, options?: RequestOptions): Promise<void>;
	...
	export class Client implements ClientSpec {
		...
		async deleteTrade(args: DeleteTradeArgs["params"], options?: RequestOptions): Promise<void> {
			return await this.makeRequest({
				method: "DELETE",
				routeId: "ContentService:ItemTransactions:deleteTrade",
				path: `/transactions/trade/trade-id/${encodeURIComponent(args.tradeId)}`,
				retryKey: options?.retryKey ?? "ContentService:ItemTransactions:deleteTrade",
			});
		}
		async getTradeRequests(options?: RequestOptions): Promise<{
			incoming: ContentServiceItemTransactions.DisplayableTradeRequest[];
			outgoing: ContentServiceItemTransactions.DisplayableTradeRequest[];
		}> {
			return await this.makeRequest({
				method: "GET",
				routeId: "ContentService:ItemTransactions:getTradeRequests",
				path: `/transactions/trade/requests`,
				retryKey: options?.retryKey ?? "ContentService:ItemTransactions:getTradeRequests",
			});
		}
		async requestTrade(
			args: RequestTradeArgs["data"],
			...
			return await this.makeRequest({
				method: "POST",
				routeId: "ContentService:ItemTransactions:requestTrade",
				path: `/transactions/trade/request`,
				retryKey: options?.retryKey ?? "ContentService:ItemTransactions:requestTrade",
				body: args,
			});
		}
		async respondToTrade(args: RespondToTradeArgs, options?: RequestOptions): Promise<void> {
			return await this.makeRequest({
				method: "POST",
				routeId: "ContentService:ItemTransactions:respondToTrade",
				path: `/transactions/trade/trade-id/${encodeURIComponent(args.params.tradeId)}`,
				retryKey: options?.retryKey ?? "ContentService:ItemTransactions:respondToTrade",
				body: args.data,
			});
		}
		async trade(
