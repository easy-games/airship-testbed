...
local TS = require("@Easy/Core/Shared/Resources/TS/Runtime/RuntimeLib")
local Modding = require("@Easy/Core/Shared/Resources/TSExtra/node_modules/@easy-games/flamework-core/out/modding").Modding
...
do
	...
	local function AddPath(path, searchPattern)
		...
		do
			...
			while true do
				...
				path = string.split(path, "assets/bundles/")[2]
	...
	local function Ignite(patchedConfig)
		...
		for ctor in Reflect.objToId do
			local allowed = false
			if isService(ctor) and isServer then
				allowed = true
			elseif isController(ctor) and isClient then
				allowed = true
			end
			if not allowed then
				continue
		...
		for _, decoratorType in { "$:flamework@Service", "$:flamework@Controller" } do
		...
		for dependency, identifier in init do
			if startedIdentifiers[identifier] ~= nil then
				continue
			end
		...
		-- });
		-- }
		for dependency, identifier in start do
			if startedIdentifiers[identifier] ~= nil then
				continue
			end
			startedIdentifiers[identifier] = true
