...
import {
  ...
  PointerButton,
  PointerDirection,
} from "@Easy/Core/Shared/Util/CanvasAPI";
...
export class FriendsController implements OnStart {
  ...
  OnStart(): void {
    const friendsContent = this.mainMenuController.refs.GetValue(
      "Social",
      "FriendsContent"
    );
    ...
    const cachedStatusesRaw = StateManager.GetString(
      "main-menu:friend-statuses"
    );
    ...
    this.socketController.On<{ initiatorId: string }>(
      "user-service/friend-requested",
      (data) => {
        print("friend-requested: " + inspect(data));
        this.FetchFriends();
        const foundUser = this.incomingFriendRequests.find(
          (u) => u.uid === data.initiatorId
        );
        if (foundUser) {
          this.socialNotification.gameObject.SetActive(false);
          this.socialNotification.gameObject.SetActive(true);
          ...
          const sprite = Airship.players.GetProfilePictureSpriteAsync(
            foundUser.uid
          );
          if (sprite) {
          ...
          AudioManager.PlayGlobal(
            "@Easy/Core/Shared/Resources/Sound/FriendRequest.wav"
          );
          if (Game.coreContext === CoreContext.GAME) {
            Game.localPlayer.SendMessage(
              ChatColor.Yellow(foundUser.username) +
                ChatColor.Gray(" sent you a friend request.")
            );
          }
        }
      }
    ...
    this.socketController.On<{ initiatorId: string }>(
      "user-service/friend-accepted",
      (data) => {
        this.FetchFriends();
      }
    );

    this.socketController.On<FriendStatus[]>(
      "game-coordinator/friend-status-update-multi",
      (data) => {
        // print("status updates: " + inspect(data));
        for (const newFriend of data) {
          const existing = this.friendStatuses.find(
    ...
    this.socketController.On(
      "game-coordinator/status-update-request",
      (data) => {
        this.SendStatusUpdate();
      }
    );
  ...
  public SetIncomingFriendRequests(friendRequests: User[]): void {
    ...
    if (count > 0) {
      this.friendRequestsButton.text.text =
  ...
  public SendStatusUpdate(): void {
    ...
    InternalHttpManager.PutAsync(
      AirshipUrl.GameCoordinator + "/user-status/self",
      EncodeJSON(status)
    );
  }
  ...
  public AcceptFriendRequestAsync(username: string, userId: string): boolean {
    ...
    if (res.success) {
      this.SetIncomingFriendRequests(
        this.incomingFriendRequests.filter((u) => u.uid !== userId)
      );
  ...
  public RejectFriendRequestAsync(userId: string): boolean {
    const res = InternalHttpManager.DeleteAsync(
      AirshipUrl.GameCoordinator + "/friends/uid/" + userId
    );
    if (res.success) {
  ...
  public GetFriendGo(uid: string): GameObject | undefined {
    return this.mainMenuController.refs
      .GetValue("Social", "FriendsContent")
      .transform.FindChild(uid)?.gameObject;
  }

  public HasOutgoingFriendRequest(userId: string): boolean {
    return (
      this.outgoingFriendRequests.find((f) => f.uid === userId) !== undefined
    );
  }
  ...
  public UpdateFriendsList(): void {
    ...
    const onlineCount = this.friendStatuses
      .filter((f) => f.status === "online")
      .size();
    const onlineCountText = this.mainMenuController.refs.GetValue(
      "Social",
      "FriendsOnlineCounter"
    ) as TMP_Text;
    ...
    for (const friend of sorted) {
      ...
      let go: GameObject | undefined = friendsContent.transform.FindChild(
        friend.userId
      )?.gameObject;
      let init = false;
      if (go === undefined) {
        go = Object.Instantiate(
          AssetCache.LoadAsset(
            "@Easy/Core/Shared/Resources/Prefabs/UI/MainMenu/Friend.prefab"
          ),
        ...
        const Teleport = () => {
          if (friend.gameId === undefined || friend.serverId === undefined)
            return;
          ...
          Dependency<TransferController>().TransferToGameAsync(
            friend.gameId,
            friend.serverId
          );
        };
    ...
    for (const renderedUid of this.renderedFriendUids) {
      if (
        this.friendStatuses.find((f) => f.userId === renderedUid) === undefined
      ) {
