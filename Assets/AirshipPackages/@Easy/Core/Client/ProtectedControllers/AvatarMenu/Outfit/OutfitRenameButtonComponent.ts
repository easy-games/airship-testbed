	...
	public Start(): void {
		CanvasAPI.OnHoverEvent(this.gameObject, (hoverState) => {
			if (hoverState === HoverState.ENTER) {
				NativeTween.GraphicColor(this.image, this.hoveredColor, 0.1).SetUseUnscaledTime(true);
			} else {
				NativeTween.GraphicColor(this.image, this.inactiveColor, 0.1).SetUseUnscaledTime(true);
			}
		});
