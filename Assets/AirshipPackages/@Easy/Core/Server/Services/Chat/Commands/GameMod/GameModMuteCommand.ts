...
export class GameModMuteCommand extends ChatCommand {
    ...
    public Execute(modPlayer: Player, args: string[]): void {
        if (args.size() === 0) {
            modPlayer.SendMessage(ChatColor.Red(`Invalid usage: /mute <username> <reason?>`));
            return;
        }
        ...
        if (action) {
            modPlayer.SendMessage(`Permanently muted ${target.username} for reason: ${action.reason}`);
        } else {
            modPlayer.SendMessage(ChatColor.Red(`Failed to permanently mute ${target.username}.`));
        }
    }
}
