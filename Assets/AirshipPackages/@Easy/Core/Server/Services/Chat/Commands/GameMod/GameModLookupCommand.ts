...
export class GameModLookupCommand extends ChatCommand {
    ...
    public Execute(modPlayer: Player, args: string[]): void {
        if (args.size() === 0) {
            modPlayer.SendMessage(ChatColor.Red(`Invalid usage: /modlookup <username>`));
            return;
        }
        ...
        if (!profile) {
            modPlayer.SendMessage(ChatColor.Red(`Moderation history not found for: ${targetUsername}`));
            return;
        }
        ...
        modPlayer.SendMessage(`------------------------------------------`);
        modPlayer.SendMessage(`Moderation profile for ${target.username}:`);
        modPlayer.SendMessage(`  Active Mute?: ${muteDisplay}`);
        modPlayer.SendMessage(`  Active Ban?: ${banDisplay}`);
        modPlayer.SendMessage(`  Past Actions?: ${actionsDisplay}`);
        modPlayer.SendMessage(`  Notes?: ${notesDisplay}`);
        modPlayer.SendMessage(`------------------------------------------`);
    }
}
