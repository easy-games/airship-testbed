...
export class World {
	public static skybox = "AirshipPackages/@Easy/Core/Skybox/BrightSky/bright_sky_2.png";
	...
	private PlaceBlockByVoxelId(pos: Vector3, blockVoxelId: number, config?: PlaceBlockConfig): void {
		...
		if (RunUtil.IsServer()) {
			SurvivalNetwork.ServerToClient.BlockPlace.server.FireAllClients(pos, blockVoxelId, config?.placedByEntityId);
		}
		if (RunUtil.IsClient()) {
