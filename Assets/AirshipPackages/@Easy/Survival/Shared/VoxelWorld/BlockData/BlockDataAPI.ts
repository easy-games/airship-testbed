...
export class BlockDataAPI {
  ...
  public static Init(): void {
    if (RunCore.IsClient()) {
      CoreNetwork.ServerToClient.SetBlockData.client.OnServerEvent(
        (blockPos, key, data) => {
          this.SetBlockData(blockPos, key, data);
        }
      );
      CoreNetwork.ServerToClient.SetBlockGroupCustomData.client.OnServerEvent(
        (blockPos, key, data) => {
          this.SetBlockGroupCustomData(blockPos, key, data);
        }
      );
      CoreNetwork.ServerToClient.SetBlockGroupSameData.client.OnServerEvent(
        (blockPos, key, data) => {
  ...
  public static SetBlockData(
    ...
    if (notifyClient && RunUtil.IsServer()) {
      CoreNetwork.ServerToClient.SetBlockData.server.FireAllClients(
        blockPos,
        key,
        data
      );
  ...
  public static SetBlockGroupCustomData(
    ...
    if (RunUtil.IsServer()) {
      CoreNetwork.ServerToClient.SetBlockGroupCustomData.server.FireAllClients(
        blockPositions,
        key,
        data
      );
  ...
  public static SetBlockGroupSameData(
    blockPositions: Vector3[],
    key: string,
    data: unknown
  ): void {
    ...
    if (RunUtil.IsServer()) {
      CoreNetwork.ServerToClient.SetBlockGroupSameData.server.FireAllClients(
        blockPositions,
        key,
        data
      );
