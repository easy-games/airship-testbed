...
export default class VoxelWorldPlantManager extends AirshipSingleton {
	...
	protected Start(): void {
		if (Game.IsServer()) {
			...
			this.bin.Add(
				VoxelWorldPlantEvents.DamagePlants.server.OnClientEvent((player, positions) => {
			...
			// CLIENT
			this.bin.Add(
				VoxelWorldPlantEvents.SetPlants.client.OnServerEvent((plantData) => {
			...
			this.bin.Add(
				VoxelWorldPlantEvents.RemovePlants.client.OnServerEvent((plantPos) => {
			...
			this.bin.Add(
				VoxelWorldPlantEvents.UpdatePlants.client.OnServerEvent((plantData) => {
	...
	private GetRandomPlant(worldPosition: Vector3) {
		...
		data.color = new Color(math.random() * 0.3, 0.8, math.random() * 0.3);
		data.fruited = math.random() >= 0.5;
	...
	public SpawnPlant(data: PlantData, notifyImmediate = true) {
		...
		const instance = Instantiate(
			this.plantTemplate,
			tilePosition.add(new Vector3(0.5, 1, 0.5)),
			Quaternion.identity,
		).GetAirshipComponent<VoxelWorldPlantView>();
		if (instance) {
			instance.Init(data);
	...
	public PlaceDirtServer(pos: Vector3) {
		pos = pos.add(new Vector3(0, 0.015, 0));
	...
	private async LoadVoxelWorld() {
		if (Game.IsEditor()) {
			const data = EditorPrefs.GetString(this.WorldSaveKey);
			print("Loading world from platform: " + data);
			this.voxelWorld.DecodeFromString(data);
		} else {
			let dataStoreWorld = await Platform.Server.DataStore.GetKey<{ saveData: string }>(this.WorldSaveKey);
			if (dataStoreWorld !== undefined) {
				print("Loading world from platform: " + dataStoreWorld.saveData);
