...
export default class CharacterForceTrigger extends AirshipBehaviour {
	...
	public Awake(): void {
		Airship.Characters.onCharacterSpawned.Connect((character) => {
			...
			character.movement.OnBeginMove((isReplay, inputData) => {
	...
	private ApplyForce(character: Character) {
		let force =
			this.forceSpace === Space.Self ? this.transform.TransformVector(this.triggerForce) : this.triggerForce;

		print(
			this.gameObject.name +
				" is applying ForceTrigger impulse: " +
				force +
				": Tick: " +
				InstanceFinder.TimeManager.LocalTick,
		);
