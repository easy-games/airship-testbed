...
export function GetItemMeta(itemType: ItemType): ItemMeta {
...
const blockIdToItemType = new Map<number, ItemType>();
for (const itemType of Object.values(ItemType)) {
	const itemMeta = GetItemMeta(itemType);
	if (itemMeta.block?.blockId !== undefined) {
		blockIdToItemType.set(itemMeta.block.blockId, itemType);
	}
}

export function GetItemTypeFromBlockId(blockId: number): ItemType | undefined {
	return blockIdToItemType.get(blockId);
}

// Assign ID to each ItemType
const itemIdToItemType = new Map<number, ItemType>();
let i = 0;
for (const itemType of Object.values(ItemType)) {
	const itemMeta = GetItemMeta(itemType);
	itemMeta.ItemType = itemType;
	itemMeta.ID = i;
	itemIdToItemType.set(i, itemType);
	i++;
}

export function GetItemTypeFromItemId(itemId: number): ItemType | undefined {
	return itemIdToItemType.get(itemId);
}

export function IsItemType(s: string): boolean {
